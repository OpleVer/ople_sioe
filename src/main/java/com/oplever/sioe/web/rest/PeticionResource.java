...
public class PeticionResource {
    ...
    @Timed
    public ResponseEntity<List<Peticion>> getAllPeticions(@ApiParam Pageable pageable, @RequestParam(required = false) String filter) {
        if ("prevencion-is-null".equals(filter)) {
            log.debug("REST request to get all Peticions where prevencion is null");
            return new ResponseEntity<>(peticionService.findAllWherePrevencionIsNull(),
                    HttpStatus.OK);
        }
        if ("evaluacion-is-null".equals(filter)) {
            log.debug("REST request to get all Peticions where evaluacion is null");
            return new ResponseEntity<>(peticionService.findAllWhereEvaluacionIsNull(),
                    HttpStatus.OK);
        }
